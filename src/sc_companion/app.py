...
from .sc_actions import execute_action
from .sc_inputmap import (
    find_user_inputmap,
...
KEYBOARD_DEFAULTS_PATH = Path(__file__).parent / "data" / "sc_keyboard_defaults.md"
...
log = logging.getLogger("sc_companion")
...
def _load_keyboard_defaults() -> str | None:
    """Prefer the user's actual Star Citizen actionmap; fall back to vendored defaults.

    The inputmap parser is currently a stub (sc_inputmap.find_user_inputmap
    returns None) until SC's actionmap XML is wired up. Until then this just
    returns the bundled sc_keyboard_defaults.md.
    """
    ...
    if inputmap_path is not None:
        ...
        if bindings:
            log.info(
                "loaded %d SC keyboard bindings from %s",
                len(bindings),
                inputmap_path,
            )
