...
from sc_companion.brain import ProposedAction
from sc_companion.sc_actions import (
    execute_action,
    is_safe_key,
    is_sc_foreground,
)
...
def test_execute_action_refuses_unsafe_keys(monkeypatch):
    monkeypatch.setattr(
        "sc_companion.sc_actions.is_sc_foreground", lambda: True
    )
...
def test_execute_action_refuses_when_sc_not_foreground(monkeypatch):
    monkeypatch.setattr(
        "sc_companion.sc_actions.is_sc_foreground", lambda: False
    )
...
def test_execute_action_presses_keys_when_safe_and_foreground(monkeypatch):
    ...
    monkeypatch.setattr(
        "sc_companion.sc_actions.time.sleep", lambda _s: None
    )
