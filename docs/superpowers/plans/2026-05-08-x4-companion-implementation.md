...
**Architecture:** All logic lives in one Python package (`sc_companion`). Components communicate via small typed interfaces (Brain, STT, TTS, Capture). The Qt main thread runs the overlay/tray; a background asyncio loop runs the conversation pipeline; the hotkey lib runs in a third thread and posts signals to bridge them. Each I/O wrapper has a real implementation and a stub for tests.
...
└── docs/
    ├── superpowers/
    │   ├── specs/2026-05-08-sc-companion-design.md
    │   └── plans/2026-05-08-sc-companion-implementation.md
    └── manual-test.md
```
...
- Create: `.gitignore`
- Create: `.env.example`
- Create: `src/sc_companion/__init__.py`
- Create: `src/sc_companion/__main__.py`
...
[project.scripts]
sc-companion = "sc_companion.__main__:main"

[tool.hatch.build.targets.wheel]
packages = ["src/sc_companion"]
...
`src/sc_companion/__init__.py`:
```python
__version__ = "0.1.0"
```

`src/sc_companion/__main__.py`:
```python
def main() -> int:
...
Run: python -m sc_companion

## Configure

Edit ~/.sc-companion/config.toml. See docs/superpowers/specs/2026-05-08-sc-companion-design.md for options.
```

- [ ] **Step 6: Install + run the smoke test**

```bash
cd /home/marku/sc-companion
python -m venv .venv
...
**Files:**
- Create: `src/sc_companion/config.py`
...
```python
from sc_companion.config import load_config
...
Expected: FAIL — `ImportError: sc_companion.config`

- [ ] **Step 3: Write the implementation**

`src/sc_companion/config.py`:
```python
import os
import tomllib
...
```bash
git add src/sc_companion/config.py tests/test_config.py
...
**Files:**
- Create: `src/sc_companion/brain.py`
...
```python
import pytest
from sc_companion.brain import ConversationHistory, StubBrain
...
```bash
git add src/sc_companion/brain.py tests/test_brain.py
...
**Files:**
- Create: `src/sc_companion/stt.py`
...
from sc_companion.stt import DeepgramSTT, StubSTT
...
```bash
git add src/sc_companion/stt.py tests/test_stt.py
...
**Files:**
- Create: `src/sc_companion/tts.py`
...
from sc_companion.tts import DeepgramTTS, StubTTS
...
```bash
git add src/sc_companion/tts.py tests/test_tts.py
...
**Files:**
- Create: `src/sc_companion/audio.py`
...
from sc_companion.audio import AudioRecorder
...
```bash
git add src/sc_companion/audio.py tests/test_audio.py
...
**Files:**
- Create: `src/sc_companion/capture.py`
...
```bash
git add src/sc_companion/capture.py tests/test_capture.py tests/fixtures/x4_screenshot.png
...
**Files:**
- Create: `src/sc_companion/minimax_brain.py`
...
import respx
from httpx import Response
from sc_companion.minimax_brain import MiniMaxBrain
...
`src/sc_companion/minimax_brain.py`:
```python
import base64
import httpx
...
```bash
git add src/sc_companion/minimax_brain.py tests/test_minimax_brain.py
...
**Files:**
- Create: `src/sc_companion/hotkey.py`
...
```python
from sc_companion.hotkey import Hotkey
...
```bash
git add src/sc_companion/hotkey.py tests/test_hotkey.py
...
from sc_companion.overlay import Overlay
...
```bash
git add src/sc_companion/overlay.py scripts/overlay_smoke.py
...
**Files:**
- Create: `src/sc_companion/tray.py`

- [ ] **Step 1: Write the implementation**

`src/sc_companion/tray.py`:
```python
...
import sys
from PySide6.QtWidgets import QApplication
from sc_companion.tray import Tray
...
```bash
git add src/sc_companion/tray.py scripts/tray_smoke.py
...
**Files:**
- Create: `src/sc_companion/app.py`
...
```bash
git add src/sc_companion/app.py docs/manual-test.md
